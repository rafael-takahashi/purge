...
A Firefox extension that lets you persistently remove any element from any webpage by Alt-clicking it.
...
- **Alt+Click** any element to remove it
- Hover highlighting shows what will be removed before you click
...
When you Alt+Click an element, the extension generates a stable CSS selector for it and stores that selector under the current site's hostname. On every subsequent visit to that site, the extension queries the DOM at load time and removes all stored elements.
...
| Action | How |
|---|---|
| Remove an element | `Alt+Click` the element |
